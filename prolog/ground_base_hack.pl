...
main :- 
    ...
    (ARGS = [Program,QueryFile,Input,Grounding,Evidence,Queries] ->
        main(Program, QueryFile, Input, Grounding, Evidence, Queries)
    ;
        halt(5)
    ).

main(Program, QueryFile, Input, Grounding, Evidence, Queries) :- 
    init(Program, QueryFile, Input, Grounding, Evidence, Queries),
